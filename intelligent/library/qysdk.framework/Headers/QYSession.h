...
// pageIndex 从1开始
- (void)GetAlarmList:(uint64_t)channelID pageSize:(int)ps pageIndex:(int)pi callBackWithArray:(void (^)(int32_t ret,NSMutableArray* array)) callback;
...
-(void) GetVideoQuality:(uint64_t) chno
               callBack:(void (^)(int32_t ret,enum QY_VIDEO_QUALITY action,int count)) callback;
